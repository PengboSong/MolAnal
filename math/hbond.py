...
def hbond(donor, hydro, acceptor):
    ...
    ang = math.degrees(math.acos(np.dot(hydro-donor, acceptor-donor)/math.sqrt(
        np.dot(hydro-donor, hydro-donor)*np.dot(acceptor-donor, acceptor-donor))))
    return dist, ang
...
def hbond_dir(coord, dirlistAll):
    ...
    for dirlist in dirlistAll:
        ...
        if spn == "sp2":
            if nbn == 2:
                normal = np.array(fitplane([coord[k]
                                            for k in range(coord.shape[0])]))[0:3]
                v1, v2 = hbond_dir_sp2_withnormal(
                    normal, *[coord[i] for i in dirlist[1:]])
                result.append(v1)
                result.append(v2)
            elif nbn == 3:
                v = hbond_dir_sp2_withoutnormal(
                    *[coord[i] for i in dirlist[1:]])
                result.append(v)
        elif spn == "sp3":
            if nbn == 3:
                v1, v2 = hbond_dir_sp3_twoadjacent(
                    *[coord[i] for i in dirlist[1:]])
                result.append(v1)
                result.append(v2)
            elif nbn == 4:
                v = hbond_dir_sp3_threeadjacent(
                    *[coord[i] for i in dirlist[1:]])
                result.append(v)
    if len(result) > 0:
        return result
    ...
    for (a, h) in donlista:
        for b in acclistb:
            ...
            if dist < HBOND_DISTANCE and ang < HBOND_DEGREE:
                ...
                don = '{0:>5}'.format(
                    mola_id) + " " + '{0:>4}'.format(mola_name) + " " + '{0:>4}'.format(mola_atom[a])
                hyd = '{0:>5}'.format(
                    mola_id) + " " + '{0:>4}'.format(mola_name) + " " + '{0:>4}'.format(mola_atom[h])
                acc = '{0:>5}'.format(
                    molb_id) + " " + '{0:>4}'.format(molb_name) + " " + '{0:>4}'.format(molb_atom[b])
                log.append(don + " | " + hyd + " | " + acc + " | " +
                           '{0:>9.3f}'.format(dist) + " | " + '{0:>6.1f}'.format(ang))
    for b in acclista:
        for (a, h) in donlistb:
            # Coordinate
            donor = molb_coord[a]
            ...
            if dist < HBOND_DISTANCE and ang < HBOND_DEGREE:
                ...
                # Detail(mol id + mol name + atom name)
                don = '{0:>5}'.format(
                    molb_id) + " " + '{0:>4}'.format(molb_name) + " " + '{0:>4}'.format(molb_atom[a])
                hyd = '{0:>5}'.format(
                    molb_id) + " " + '{0:>4}'.format(molb_name) + " " + '{0:>4}'.format(molb_atom[h])
                acc = '{0:>5}'.format(
                    mola_id) + " " + '{0:>4}'.format(mola_name) + " " + '{0:>4}'.format(mola_atom[b])
                log.append(don + " | " + hyd + " | " + acc + " | " +
                           '{0:>9.3f}'.format(dist) + " | " + '{0:>6.1f}'.format(ang))
    return hbondn, log
        ...
        for b in acclistb:
            ...
            if dist < HBOND_DISTANCE and ang < HBOND_DEGREE:
                ...
                if withDirection is True:
                    for d in directionb:
                        ang_d = math.degrees(math.acos(
                            np.dot(hydro-acceptor, d)/math.sqrt(np.dot(hydro-acceptor, hydro-acceptor))))
                        if ang_d < HBOND_DIRDEGREE:
                            isHydroBond = True
                            ang_ds.append(ang_d)
                            break
                ...
                if isHydroBond is True:
                    ...
                    acc = '{0:>5}'.format(
                        molb_id) + " " + '{0:>4}'.format(molb_name) + " " + '{0:>4}'.format(molb_atom[b])
                    log.append(don + " | " + hyd + " | " + acc + " | " + '{0:>9.3f}'.format(
                        dist) + " | " + '{0:>6.1f}'.format(ang) + ", " + '{0:>6.1f}'.format(ang_d))
...
def hbondmolgrp(prof, framecoord, moltypeA, moltypeB, lpdir=False):
    ...
    for mola_id in grpA:
        ...
        if 10 * molan < molbn:
            ...
            molb_coords = [np.vstack(
                [framecoord[l-1] for l in prof.get(id).get("atomid")]) for id in grpB]
            if lpdir is True:
                hbondns_single, log, directiondb = hbondmol_dir_multiple(
                    molA, molb_name, molb_num, molb_ids, molb_atoms, molb_coords, log, directiondb)
            else:
                hbondns_single, log = hbondmol_multiple(
                    molA, molb_name, molb_num, molb_ids, molb_atoms, molb_coords, log)
            ...
            for molb_id in grpB:
                ...
                if lpdir is True:
                    hbondn, log, directiondb = hbondmol_dir(
                        molA, molB, log, directiondb)
                else:

...
def readline_csv_spec(line, row):
	...
	if isinstance(splitLine, list) and len(splitLine) == 8:
		...
		if isinstance(convert_input_type(new_id), int):
			...
			raise ValueError("Wrong data type. Term Newline at row " + rowText + " column 1 is not an integer.")
		if new_id != 0:
			if isinstance(convert_input_type(old_id), int):
				old_id = convert_input_type(old_id)
			else:
				raise ValueError("Wrong data type. Term Oldline at row " + rowText + " column 2 is not an integer.")
			if isinstance(convert_input_type(atom_name), str):
				atom_name = convert_input_type(atom_name)
			else:
				raise ValueError("Wrong data type. Term AtomName at row " + rowText + " column 3 is not characters.")
			if isinstance(convert_input_type(res_type), str):
				res_type = convert_input_type(res_type)
			else:
				raise ValueError("Wrong data type. Term ResType at row " + rowText + " column 4 is not characters.")
			if isinstance(convert_input_type(res_id), int):
				res_id = convert_input_type(res_id)
			else:
				raise ValueError("Wrong data type. Term ResId at row " + rowText + " column 5 is not an integer.")
			if isinstance(convert_input_type(occup), float):
				occup = convert_input_type(occup)
			else:
				raise ValueError("Wrong data type. Term Occup at row " + rowText + " column 6 is not a real number.")
			if isinstance(convert_input_type(temp_factor), float):
				temp_factor = convert_input_type(temp_factor)
			else:
				raise ValueError("Wrong data type. Term TempFactor at row " + rowText + " column 7 is not a real number.")
			if isinstance(convert_input_type(segment), str):
				segment = convert_input_type(segment)
			else:
				raise ValueError("Wrong data type. Term SegName at row " + rowText + " column 8 is not characters.")
